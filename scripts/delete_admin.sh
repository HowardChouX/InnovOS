#!/usr/bin/env bash
# 将管理员降级为普通用户（撤销超级用户权限）。
# 用法：make delete-admin  （交互式输入手机号）
#
# 仅对已注册且当前是管理员的账户生效。
set -euo pipefail
...
if [[ "$is_super" != "t" && "$is_super" != "true" ]]; then
  echo "ℹ️  用户 $uname (id=$uid) 本就是普通用户，无需操作。"
  exit 0
fi

# ── 降级 ──
"${PSQL[@]}" -c "UPDATE users SET is_superuser = FALSE WHERE id = $uid" >/dev/null

echo "✅ 已将用户 $uname (id=$uid, phone=$phone) 降级为普通用户。"
