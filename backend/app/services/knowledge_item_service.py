...
class KnowledgeItemService:
    ...
    @staticmethod
    def list(base_id: str, page: int = 1, limit: int = 20, type: str = None, groupId: str = None) -> dict:
        """分页列出知识项"""
        db = get_db()
        offset = (page - 1) * limit
    ...
    @staticmethod
    def get_items_by_base_id(base_id: str, groupId: str = None) -> list[dict]:
        """获取知识库下的所有知识项（非分页）"""
        db = get_db()
        conditions = ["base_id = ?", "status != 'deleting'"]
        params = [base_id]
    ...
    @staticmethod
    def get_root_items_by_base_id(base_id: str) -> list[dict]:
        """获取知识库的根项"""
        return KnowledgeItemService.get_items_by_base_id(base_id, groupId="null")

    @staticmethod
    def get_outermostSelectedItem_ids(base_id: str, item_ids: list[str]) -> list[str]:
        """获取最外层选中的项 ID（排除子项）"""
        ...
        for item_id in selected_ids:
            descendants = KnowledgeItemService.get_subtree_items(base_id, [item_id])
    ...
    @staticmethod
    def get_deleting_root_groups() -> list[dict]:
        """获取正在删除的根组（用于崩溃恢复）"""
        db = get_db()
        rows = db.execute("""
    ...
    @staticmethod
    def create(base_id: str, item: dict) -> Optional[dict]:
        """创建知识项"""
        db = get_db()
    ...
    @staticmethod
    def set_subtree_status(base_id: str, root_ids: list[str], status: str, error: str = None) -> list[str]:
        """递归设置子树状态（使用递归 CTE）"""
        ...
        """, [base_id, *unique_root_ids, base_id, status, error or None, base_id] +
        ...
        if status == "failed":
            ...
            KnowledgeItemService._reconcile_containers(base_id, parent_ids)

        db.close()
        return updated_ids

    @staticmethod
    def delete_items_by_ids(base_id: str, item_ids: list[str]) -> None:
        """批量删除知识项"""
        ...
        # 协调容器状态
        KnowledgeItemService._reconcile_containers(base_id, group_ids)

    @staticmethod
    def get_subtree_items(base_id: str, root_ids: list[str], include_roots: bool = False, leaf_only: bool = False) -> list[dict]:
        """获取子树项（递归 CTE）"""
        ...
        rows = db.execute(f"""
            ...
            FROM subtree
            INNER JOIN knowledge_item item ON item.id = subtree.id AND item.base_id = ?
    ...
    @staticmethod
    def update_status(item_id: str, status: str, error: str = None) -> Optional[dict]:
        """更新知识项状态"""
        db = get_db()
        existing = db.execute("SELECT * FROM knowledge_items WHERE id=?", (item_id,)).fetchone()
        if not existing:
            db.close()
            return None
        ...
        KnowledgeItemService._reconcile_containers(row["base_id"], start_containers)
        return KnowledgeItemService._row_to_item(row)
    ...
    def delete(item_id: str) -> bool:
        ...
        # 协调容器状态
        KnowledgeItemService._reconcile_containers(base_id, [group_id])
        return True

    @staticmethod
    def _reconcile_containers(base_id: str, start_container_ids: list) -> None:
        """协调容器状态（从子项状态推导）"""
        db = get_db()
        ...
        while queue:
            ...
            if active_count > 0:
                db.execute(
                    "UPDATE knowledge_items SET status='processing', error=NULL, updated_at=datetime('now') WHERE base_id=? AND id=?",
                    (base_id, container_id),
                )
                db.commit()
                if container["group_id"]:
            ...
            db.execute(
                "UPDATE knowledge_items SET status=?, error=?, updated_at=datetime('now') WHERE base_id=? AND id=?",
                (next_status, next_error, base_id, container_id),
            )
            db.commit()
...
class KnowledgeItemServiceCompat:
    ...
    def list_items(user_id, base_id, page=1, limit=20, item_type="", group_id=""):
        return KnowledgeItemService.list(base_id, page, limit, item_type or None, group_id or None)

    @staticmethod
    def get_by_id(user_id, item_id):
        return KnowledgeItemService.get_by_id(item_id)

    @staticmethod
    def create_item(user_id, base_id, data):
        return KnowledgeItemService.create(base_id, data)

    @staticmethod
    def update_status(user_id, item_id, status, error=""):
        return KnowledgeItemService.update_status(item_id, status, error)

    @staticmethod
    def delete_item(user_id, item_id):
        return KnowledgeItemService.delete(item_id)

    @staticmethod
    def delete_items_by_base(user_id, base_id, item_ids):
        KnowledgeItemService.delete_items_by_ids(base_id, item_ids)
        return True
