...
@router.post("")
def create_notification(body: CreateNotificationInput, user: dict = Depends(get_current_user)):
    if user["role"] != "admin":
...
@router.post("/batch")
def batch_send_notification(body: BatchNotificationInput, user: dict = Depends(get_current_user)):
    if user["role"] != "admin":
        raise HTTPException(status_code=403, detail="Admin only")
