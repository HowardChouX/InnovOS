...
def row_to_dict(row: dict) -> dict:
    ...
    return {
        ...
        "lastUsedAt": utc_iso(row.get("last_used_at")),
...
def toggle_key(key_id: int, user: dict = Depends(require_admin)):
    """启用/禁用API Key（仅管理员）"""
    key = key_manager.get_key_by_id(key_id)
    if not key:
        raise HTTPException(status_code=404, detail="Key不存在")
    new_status = not key["is_active"]
    key_manager.update_key(key_id, is_active=new_status)
    return {"data": {"id": key_id, "is_active": new_status}, "message": "状态已切换"}
