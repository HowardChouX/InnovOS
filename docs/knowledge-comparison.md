> ⚠️ **历史文档**：本文档对比 CherryStudio 与 InnovOS 知识库架构。当前 InnovOS 已从 numpy 向量存储迁移到 PostgreSQL pgvector，并完善了作业系统（5 种 job types）、锁管理、重排等功能。部分对比已不准确。
...
- InnovOS: 已实现 5 个作业处理器（backend/app/services/knowledge_jobs/）

### 7. Vector Store

- cherry-studio: 使用 LibSQL 向量存储
- InnovOS: 使用 PostgreSQL pgvector

### 8. Rerank

- cherry-studio: 支持多种重排适配器
- InnovOS: 已实现 reranker.py（`backend/app/algorithm/knowledge/reranker.py`）
