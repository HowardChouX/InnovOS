...
> **说明**：本文档为产品愿景与技术规格描述。当前后端 `/api/evaluation` 接口通过 AI 进行实时四维评估（创新性、可行性、完整性、转化潜力），非 mock 数据。以下内容描述了 ML 驱动的评估引擎迭代目标。

> ⚠️ **本文档描述的是四维评估引擎的产品愿景。实际 `evaluations` 表使用 TRIZ 进化评估模式（列：`root_cause_cut`, `ifr_distance`, `evolution_alignment` 等），详情见 `pg_schema.py`。**
...
```json
{
  "comprehensive_score": {
    ...
    "strengths": ["创新性突出，与现有专利差异化明显", "推理过程完整，逻辑清晰"],
    "weaknesses": ["转化路径不够清晰", "成本分析不够详细"]
  }
}
```
