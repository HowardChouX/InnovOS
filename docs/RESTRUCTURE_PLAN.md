> ⚠️ **历史文档**：本文档记录的是 v3 → v4 重构计划。当前项目已基本完成重构，部分描述可能与最新代码有出入。最新状态请参考 `docs/DEVELOPMENT_GUIDE.md`。
...
| 向量存储 | `knowledge/vectorstore/` (LibSQL)   | PostgreSQL + pgvector                       |
...
| 数据隔离     | PostgreSQL `user_id` 字段 + 查询过滤 | 与现有架构一致，简洁有效                       |
