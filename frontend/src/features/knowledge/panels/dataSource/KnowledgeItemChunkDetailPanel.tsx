...
export default function KnowledgeItemChunkDetailPanel({
  ...
  const typeMeta =
    item && viewModel ? viewModel.suffix || dataSourceTypeDisplayConfig[item.type].filterLabel : '';
  ...
  useEffect(() => {
    ...
    const loadChunks = async () => {
      ...
      try {
        const res = await knowledgeApi.listItemChunks(baseId, itemId);
        if (isActive) {
          const currentItem = fetchedItem ?? initialItem;
          const itemType = currentItem?.type ?? 'file';
          const itemData = currentItem?.data;
          const source =
            itemData && 'source' in itemData ? (itemData as { source: string }).source : '';
          setChunks(
            res.data.map((chunk) => ({
              id: chunk.id,
              itemId,
  ...
  }, [baseId, itemId, fetchedItem, initialItem]);
  ...
  const handleConfirmDeleteChunk = async () => {
    ...
    try {
      await knowledgeApi.deleteItemChunk(baseId, chunk.itemId, chunk.id);
      setChunks((currentChunks) =>
        currentChunks.filter((currentChunk) => currentChunk.id !== chunk.id),
      );
    ...
    <div className="flex h-full min-h-0 flex-col">
          ...
          <div className="flex items-center gap-2 text-xs leading-4 text-foreground-muted">
            {metaParts.map((part) => (
              <span
                key={part}
                className={
                  viewModel && part === typeMeta && viewModel.suffix ? 'uppercase' : undefined
                }
              >
                {part}
              </span>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-4 py-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {isItemLoading || isLoading ? (
          <KnowledgeItemChunkState>加载中...</KnowledgeItemChunkState>
        ) : null}
        {!isItemLoading && itemError ? (
          <KnowledgeItemChunkState>{itemError.message}</KnowledgeItemChunkState>
        ) : null}
