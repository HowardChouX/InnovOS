...
import { RecallTestContext } from './useRecallTest';
...
export default function RecallTestProvider({ baseId, children }: RecallTestProviderProps) {
  return (
    <RecallTestProviderInner key={baseId} baseId={baseId}>
      {children}
    </RecallTestProviderInner>
  );
}

function RecallTestProviderInner({ baseId, children }: RecallTestProviderProps) {
  const latestSearchIdRef = useRef(0);
  const [query, setQuery] = useState('');
  const [historyQueriesByBaseId, setHistoryQueriesByBaseId] =
    useState<Record<string, string[]>>(loadHistoryQueries);
  ...
  const topScore =
    scoreKind === 'relevance' ? results.reduce((score, item) => Math.max(score, item.score), 0) : 0;
