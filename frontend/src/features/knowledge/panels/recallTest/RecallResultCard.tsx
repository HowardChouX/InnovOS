    ...
    <div className="group/chunk rounded-md border border-border-subtle bg-background transition-all hover:border-border-hover">
      <div className="flex items-center gap-2 px-3 py-2">
        ...
        <div className="flex min-w-0 flex-1 items-center gap-1">
          ...
          <span className="shrink-0 text-xs leading-3 text-foreground-muted">
            #{item.chunkIndex}
          </span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="w-16 text-right text-xs leading-4 text-foreground-muted tabular-nums">
            {scoreLabel}
          </span>
        </div>
        <button
      ...
      <div className="overflow-hidden px-3 pb-3">
        <p
          className={`text-sm leading-relaxed text-foreground-secondary ${isExpanded ? '' : 'line-clamp-2'}`}
        >
          {item.content}
        </p>
      </div>
