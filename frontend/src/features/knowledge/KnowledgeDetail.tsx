...
export function KnowledgeDetail() {
  ...
  const addButtonLabel =
    activeTab === 'file' ? '添加文件' : activeTab === 'note' ? '添加笔记' : '添加网址';
          ...
          <div className="flex min-w-0 items-start gap-3">
            ...
            <div className="flex min-w-0 flex-col gap-1.5">
              <div className="flex min-w-0 items-center gap-2">
                ...
                <span
                  className={`shrink-0 rounded-full border px-2 py-0.5 text-[10px] font-medium ${base.status === 'completed' ? 'badge-success' : 'badge-danger'}`}
                >
              ...
              <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-foreground-muted leading-4">
                <span>{base.itemCount} 个数据源</span>
                <span aria-hidden="true">|</span>
                <span>更新于 {new Date(base.updatedAt).toLocaleDateString()}</span>
              </div>
            </div>
          </div>
          <div className="flex shrink-0 items-center gap-1">
            <button
              onClick={openRecallTest}
              title="召回测试"
              className="flex h-8 items-center justify-center rounded-md text-foreground-muted hover:bg-accent"
            >
        ...
        {SOURCE_TABS.map((tab) => {
          ...
          return (
            ...
            >
              ...
              {tab.label}
              <span
                className={`rounded-full px-1.5 py-0.5 text-[10px] min-w-[16px] text-center ${isActive ? 'bg-primary/15 text-primary' : 'bg-background-muted text-foreground-muted'}`}
              >
                {tabCounts[tab.key]}
              </span>
            </button>
          ...
          <div className="flex flex-col gap-1.5">
            {filteredItems.map((item) => (
              <DocRow
                ...
                onClick={() => openItemChunks(item.id)}
              />
            ))}
          </div>
        )}
...
function DocRow({
  ...
  return (
    ...
    >
      ...
      <span
        className={`inline-flex w-[5em] items-center justify-center gap-1 text-xs ${status.color}`}
      >
        <i className={`${status.icon} text-[10px]`} />
        {status.label}
      </span>
      {reindexing ? (
        <span className="flex h-7 w-7 items-center justify-center">
          <i className="fa-solid fa-spinner fa-spin text-xs text-foreground-muted" />
        </span>
      ) : (
        <>
          {canReindex ? (
