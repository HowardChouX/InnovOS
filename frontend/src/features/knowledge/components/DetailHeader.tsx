...
const DetailHeader = ({
  ...
  const formattedDate = base.updatedAt ? new Date(base.updatedAt).toLocaleDateString('zh-CN') : '';
  ...
  const statusBadge = isFailed ? 'badge-danger' : 'badge-success';
          ...
          <div className="flex min-w-0 items-start gap-3">
            ...
            <div className="flex min-w-0 flex-col gap-1.5">
              <div className="flex min-w-0 items-center gap-2">
                ...
                <span
                  className={`shrink-0 rounded-full border px-2 py-0.5 text-[10px] font-medium ${statusBadge}`}
                >
                  {isFailed ? '失败' : '正常'}
                </span>
              </div>
