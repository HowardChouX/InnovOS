...
export function KeyManagementPage() {
  ...
  const [assignedModels, setAssignedModels] = useState<{
    ...
    vision: string;
    extract: string;
  }>({ chat: '', embedding: '', rerank: '', vision: '', extract: '' });
  ...
  const load = async () => {
    ...
    try {
      ...
      console.error('[KeyManagementPage] load providers failed:', e);
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    providersApi
      ...
      .catch((e) => console.error('[KeyManagementPage] load providers failed:', e))
  ...
  useEffect(() => {
    settingsApi
      ...
      .catch((e) => {
        console.error('[KeyManagementPage] getAvailable failed:', e);
      });
    settingsApi
      .getAssigned()
      .then((r) => {
      ...
      .catch((e) => {
        console.error('[KeyManagementPage] getAssigned failed:', e);
      });
  }, []);
  ...
  useEffect(() => {
    ...
    if (p) {
      const models = (p.models || []).map(getModelId);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const data: Record<string, unknown> = { max_rpm: editMaxRpm, models: editModels };
      ...
      console.error('[KeyManagementPage] save failed:', e);
      setToast({ msg: e instanceof Error ? e.message : '保存失败', type: 'error' });
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleCheck = (model?: string) => {
    ...
    providersApi
      .check(selected.providerId, testModel)
      .then((r) => {
        setToast(
          r.data.status === 'ok'
      ...
      .catch((e) => {
        console.error('[KeyManagementPage] check failed:', e);
        setToast({ msg: `${testModel} 连接失败`, type: 'error' });
      })
  ...
  const handleToggle = async () => {
    if (!selected) return;
    try {
      await providersApi.update(selected.providerId, { is_enabled: !selected.isEnabled });
      await load();
    } catch (e) {
      console.error('[KeyManagementPage] toggle failed:', e);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await providersApi.delete(selected.providerId);
      setSelectedId(null);
      await load();
    } catch (e) {
      console.error('[KeyManagementPage] delete failed:', e);
  ...
  const handleDetectModels = async () => {
    ...
    setDetectedModels([]);
    try {
      const res = await providersApi.detectModels(selected.providerId);
      const models = res.data?.models || [];
      setDetectedModels(models);
      if (models.length > 0) setShowModelSelector(true);
      else setToast({ msg: '未检测到模型', type: 'error' });
    } catch (e) {
      console.error('[KeyManagementPage] detectModels failed:', e);
      setToast({ msg: '检测失败', type: 'error' });
    } finally {
      setDetecting(false);
    }
  };
  ...
  const handleBatchCheck = async () => {
    ...
    try {
      ...
      if (failed.length === 0)
        setToast({ msg: `全部 ${res.data.models.length} 个模型检查通过`, type: 'success' });
      else
        setToast({
          msg: `${failed.length}/${res.data.models.length} 个模型连接失败`,
          type: 'error',
        });
    } catch (e) {
      console.error('[KeyManagementPage] batchCheck failed:', e);
      setToast({ msg: '批量检查失败', type: 'error' });
    } finally {
  ...
  const handleSaveAssignment = async () => {
    ...
    try {
      ...
      console.error('[KeyManagementPage] saveAssignment failed:', e);
      setToast({ msg: '保存模型分配失败', type: 'error' });
    } finally {
      setSavingAssignment(false);
    }
  };

  const [newP, setNewP] = useState({ provider_id: '', name: '', api_host: '' });
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      setShowAdd(false);
      setNewP({ provider_id: '', name: '', api_host: '' });
      await load();
    } catch (e) {
      console.error('[KeyManagementPage] add provider failed:', e);
      setToast({ msg: e instanceof Error ? e.message : '添加失败', type: 'error' });
    } finally {
      setAdding(false);
    }
  };
  ...
  return (
    <div style={{ display: 'flex', flex: 1, minHeight: 0, gap: 0, height: '100%' }}>
              ...
              >
                <div
                  style={{
                    ...
                    height: 28,
                    borderRadius: 6,
                    background: p.isConfigured ? 'rgba(74,222,128,0.12)' : 'rgba(255,255,255,0.06)',
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
                    fontSize: 12,
                    fontWeight: 700,
                    color: p.isConfigured ? 'var(--accent-green)' : 'var(--text-tertiary)',
                  }}
                >
                  {p.name[0]}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 10, color: 'var(--text-tertiary)', marginTop: 1 }}>
                    {p.isConfigured ? p.apiKeyMasked || '已配置' : '未配置'}
          ...
          <div style={{ maxWidth: 520 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
              <div
                style={{
                  ...
                  height: 40,
                  borderRadius: 10,
                  background: selected.isConfigured
                  ...
                  fontSize: 18,
                  fontWeight: 700,
                  color: selected.isConfigured ? 'var(--accent-green)' : 'var(--text-tertiary)',
                }}
              >
                {selected.name[0]}
              </div>
                ...
                {selected.isConfigured && editModels.length > 1 && (
                  <button
                    ...
                    style={{
                      ...
                      cursor: 'pointer',
                      fontFamily: 'inherit',
                      opacity: batchChecking ? 0.5 : 1,
                    }}
                  >
                    {batchChecking ? '检查中...' : '批量检查'}
                  </button>
                )}
                <button
                  ...
                  style={{
                    ...
                    border: `1px solid ${selected.isEnabled ? 'rgba(74,222,128,0.3)' : 'rgba(248,113,113,0.3)'}`,
                    color: selected.isEnabled ? 'var(--accent-green)' : 'var(--accent-red)',
                    cursor: 'pointer',
                    fontFamily: 'inherit',
                  }}
                >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                ...
                >
                  <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>状态</span>
                  <span
                    style={{
                      fontSize: 11,
                      padding: '2px 8px',
                      borderRadius: 4,
                      background: selected.hasApiKey
              ...
              <div>
                ...
                >
                  <label style={{ ...labelStyle, marginBottom: 0 }}>
                    模型 ({editModels.length})
                  </label>
                  <button
                    onClick={handleDetectModels}
                    disabled={detecting || !selected.isConfigured}
                    style={{
                      padding: '3px 10px',
                      borderRadius: 4,
                      fontSize: 11,
                      ...
                      color: 'var(--accent-blue)',
                      fontFamily: 'inherit',
                      opacity: detecting || !selected.isConfigured ? 0.5 : 1,
                    }}
                  >
                    {detecting ? '检测中...' : '模型检测'}
                  </button>
                ...
                {editModels.length > 0 ? (
                  ...
                  >
                    {editModels.map((mId) => {
                      ...
                      const caps: string[] =
                        (typeof entry === 'object' && entry?.capabilities) || [];
                      ...
                      return (
                        <div
                          key={mId}
                          onClick={() => setEditDrawerModel(entry)}
                          style={{
                            display: 'flex',
                            alignItems: 'center',
                            justifyContent: 'space-between',
                        ...
                        >
                          ...
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              setEditModels((prev) => prev.filter((x) => x !== mId));
                            }}
                            style={{
                              background: 'none',
                              border: 'none',
              ...
              <div>
                ...
                <input
                  type="number"
                  value={editMaxRpm}
                  onChange={(e) => setEditMaxRpm(Number(e.target.value))}
                  style={inputStyle}
                />
              </div>
                ...
                {(['chat', 'embedding', 'rerank', 'vision', 'extract'] as const).map((cap) => {
                  ...
                  return (
                    <div key={cap}>
                      ...
                      <select
                        value={assignedModels[cap]}
                        onChange={(e) =>
                          setAssignedModels((p) => ({ ...p, [cap]: e.target.value }))
                        }
                        style={{
                          ...inputStyle,
                          appearance: 'auto',
            ...
            >
              ...
              {[
                ...
                <div key={f.key} style={{ marginBottom: 12 }}>
                  ...
                  <input
                    type="text"
                    value={newP[f.key as keyof typeof newP]}
              ...
              <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 16 }}>
                ...
                <button
                  onClick={handleAdd}
                  disabled={adding || !newP.provider_id || !newP.name || !newP.api_host}
                  style={{
                    padding: '7px 16px',
                    borderRadius: 6,
                    fontSize: 13,
                    ...
                    cursor: 'pointer',
                    fontFamily: 'inherit',
                    opacity: adding || !newP.provider_id || !newP.name || !newP.api_host ? 0.5 : 1,
                  }}
                >
                  {adding ? '添加中...' : '添加'}
                </button>
      ...
      <ModelSelector
        ...
        title={`${selected?.name || ''} 模型`}
      />

      {/* Model Edit Drawer */}
      <ModelEditDrawer
        open={editDrawerModel !== null}
        onClose={() => setEditDrawerModel(null)}
          ...
          >
            ...
            >
              ...
              >
                {(editModels.length > 0
                  ? editModels
                  : (selected?.models || []).map(getModelId)
                ).map((m) => (
                  <div
                    key={m}
              ...
              <button
                onClick={() => setShowCheckModelPicker(false)}
                style={{
                  padding: '7px 16px',
                  borderRadius: 6,
                  fontSize: 13,
...
function RagGlobalConfig() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await settingsApi.setRagConfig(payload as Partial<RagConfig>);
    } catch (e) {
  ...
  if (!loaded)
    return <div style={{ fontSize: 12, color: 'var(--text-tertiary)', padding: 8 }}>加载中...</div>;
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        ...
        <div style={ragFieldStyle}>
          ...
          <select
            ...
            onChange={(e) => set('search_mode', e.target.value)}
          >
        ...
        <div style={ragFieldStyle}>
          ...
          <input
            ...
            onChange={(e) => set('document_count', e.target.value)}
            placeholder="10"
          />
