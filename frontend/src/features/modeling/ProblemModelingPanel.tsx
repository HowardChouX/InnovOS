...
import type { ProblemModeling, Conflict, InnovationDirection } from '../../types/modeling';
    ...
    <div className="card" style={{ minHeight: 300 }}>
      ...
      >
        <div>
          <i
            className="fa-solid fa-cube"
            style={{ marginRight: 8, color: 'var(--accent-purple)' }}
          />
          问题建模
        </div>
        <span style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>
          {modeling.modelStructure?.problemType || '未知'} |{' '}
          {modeling.modelStructure?.complexity || '未知'}
        </span>
      </div>
            ...
            >
              ...
              {isCompleted && (
                <i
                  ...
                  style={{ fontSize: 8, color: 'var(--accent-green)' }}
                />
              )}
            </button>
          );
      ...
      <div style={{ minHeight: 200 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            >
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {modeling.problemElements?.potentialConflicts?.map(
                  (conflict: { id: string; label: string; description?: string }) => (
                    <div
                      key={conflict.id}
                      style={{
                        display: 'flex',
        ...
        {activeTab === 'conflicts' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {modeling.conflicts?.length > 0 ? (
              modeling.conflicts.map((conflict: Conflict, index: number) => (
                <div
                  key={index}
                  style={{
                    background: 'rgba(0,0,0,0.2)',
                ...
                >
                  ...
                  >
                    <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)' }}>
                      <i
                        ...
                        style={{ marginRight: 6, color: 'var(--accent-yellow)' }}
                      />
                      {conflict.type}
                    </div>
                    <span
                      style={{
                        ...
                        color:
                          conflict.severity === '高' ? 'var(--accent-red)' : 'var(--accent-yellow)',
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    {conflict.parameters?.map(
                      (
                        param: { name: string; direction?: string; requirement?: string },
                        i: number,
                      ) => (
                        <div
                          key={i}
                        ...
                        >
                          <span style={{ fontSize: 11, color: 'var(--accent-blue)' }}>
                            {param.name}
                            {param.direction && (
                              <span style={{ marginLeft: 4, opacity: 0.7 }}>{param.direction}</span>
                            )}
                            {param.requirement && (
        ...
        {activeTab === 'principles' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {modeling.recommendedPrinciples?.length > 0 ? (
              modeling.recommendedPrinciples.map((principle: string, index: number) => (
                <div
                  key={index}
                  style={{
                    display: 'flex',
        ...
        {activeTab === 'directions' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {modeling.innovationDirections?.length > 0 ? (
              modeling.innovationDirections.map((dir: InnovationDirection, index: number) => (
                <div
                  key={index}
                  style={{
                    background: 'rgba(0,0,0,0.2)',
                  ...
                  >
                    <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)' }}>
                      <i
                        ...
                        style={{ marginRight: 6, color: 'var(--accent-cyan)' }}
                      />
                      {dir.direction}
                    </div>
