...
function inferCapabilities(modelId: string): string[] {
  ...
  if (/^text-|embed|bge-|bce-|e5-|gte-|voyage-|jina-emb|text-embedding/.test(lower))
    return ['embedding'];
  return ['chat'];
}
...
export function ModelSelector({
  ...
  const [selected, setSelected] = useState<Set<string>>(new Set(selectedModels.map(getModelId)));
              ...
              return (
                <div key={group.name} style={{ marginBottom: 4 }}>
                  <div
                    ...
                    onClick={() =>
                      allSelected ? removeAllFromGroup(group.models) : addAllFromGroup(group.models)
                    }
                  >
                    ...
                    >
                      {group.models.filter((m) => selected.has(getModelId(m))).length}/
                      {group.models.length}
                    </span>
                    <span
                      style={{ marginLeft: 'auto', fontSize: 16, color: 'var(--text-secondary)' }}
                    >
                      {allSelected ? '−' : '+'}
                    </span>
                  </div>
