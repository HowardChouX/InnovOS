...
export function ModelEditDrawer({ open, onClose, providerId, model, onSave }: Props) {
  const [enabled, setEnabled] = useState(() => {
    if (model && typeof model !== 'string') return model.isEnabled ?? true;
    return true;
  });
  const [saving, setSaving] = useState(false);

  if (!open || !model || typeof model === 'string') return null;
      ...
      >
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
            ...
            {sortedCaps.length === 0 && (
              <span style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>无能力信息</span>
            )}
          </div>
        </div>
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                cursor: 'pointer',
                position: 'relative',
                background: enabled ? 'rgba(74,222,128,0.3)' : 'rgba(255,255,255,0.1)',
