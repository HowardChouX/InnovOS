...
const RichEditor = ({
  ...
  const { editor, markdown, html, formattingState, setMarkdown, setHtml, clear } = useRichEditor({
    initialContent,
    ...
    () => ({
      ...
      executeCommand: (command: string, value?: unknown) => {
        if (editor?.commands && command in editor.commands) {
          (editor.commands as unknown as Record<string, (...args: unknown[]) => void>)[command](
            value,
          );
        }
      },
